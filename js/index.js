...
//4. Spatie na voornamen weghalen // AF
//5. Artists -> Slachtoffers
//6. Meerdere voornamen in array json en die kunnen matchen
//7. ID's aan elkaar koppelen (links)
//8. Styling van ontwerp toepassen front-end


//Vragen
//
//1. Hoe kunnen we de nodes het meest efficient aan elkaar linken
//2. Intro nodig met uitleg etc.
//3. Automatisch width + height container element (D3 (SVG))
//4. Snel filteren op onderwerp ipv dataset regel 598
//5. Wat zouden jullie nog willen zien voor maandag?
...
Network = function () {
    ...
    var containerX = $('.container-main').width,
        containerY = $('.container-main').height;
    ...
    update = function () {
        ...
        /* START MEGA FILTERSORTEER DING */

        var selectButtonValue = document.getElementById("song_select").value
        console.log(selectButtonValue)
        ...
        moveToRadialLayout = function (alpha) {
            ...
            return function (d) {
                var centerNode;
                // belangrijk voor sorteren
                centerNode = groupCenters(d.selectButtonValue);
                d.x += (centerNode.x - d.x) * k;
                return d.y += (centerNode.y - d.y) * k;
            };
        };

        /* END MEGA FILTERSORTEER DING */
        ...
        return node.each(function (d) {
            ...
            if (searchTerm.length > 0 && match >= 0) {
                ...
                return element.style("fill", function (d) {
                    return nodeColors(d.achternaam);
    ...
    sortedArtists = function (nodes, links) {
        ...
        if (sort === "links") {
            ...
            links.forEach(function (l) {
                var name, name1;
                if (counts[name = l.source.achternaam] == null) {
                    counts[name] = 0;
                }
                counts[l.source.achternaam] += 1;
                if (counts[name1 = l.target.achternaam] == null) {
                    counts[name1] = 0;
                }
                return counts[l.target.achternaam] += 1;
            });
            nodes.forEach(function (n) {
                var name;
                return counts[name = n.achternaam] != null ? counts[name] : counts[name] = 0;
            });
            ...
            counts = nodeCounts(nodes, "achternaam");
            artists = d3.entries(counts).sort(function (a, b) {
                return b.value - a.value;
            });
            artists = artists.map(function (v) {
    ...
    updateCenters = function (artists) {
        if (layout === "radial") {
            return groupCenters = RadialPlacement().center({
                "x": width / 2,
                "y": height / 2 - 100
            }).radius(200).increment(18).keys(artists);
        }
    };
    ...
    strokeFor = function (d) {
        return d3.rgb(nodeColors(d.achternaam)).darker().toString();
    };
    showDetails = function (d, i) {
        var content,
            leeftijd = Math.floor(d.leeftijd / 365);
        content = '<p class="main">' + d.voornamen + ' ' + d.achternaam + '</span></p>';
        content += '<hr class="tooltip-hr">';
        content += '<p class="main">' + d.status + ' ' + leeftijd + '</span></p>';
...
$(function () {
    ...
    ////          var newLayout2;
    ////    newLayout = d3.select(this).attr("id");
    ////    activate("layouts", newLayout2);
    ////    return myNetwork.toggleLayout(newLayout2);
    //
    //
    ////      var newSort2;
    ////    newSort2 = $(this).val();
    ////    activate("sorts", newSort2);
    ////    return myNetwork.toggleSort(newSort2);
    //      var selectButtonValue = document.getElementById("song_select").value;
    //console.log(selectButtonValue);
    //
    //        moveToRadialLayout = function(alpha) {
    ...
    return d3.json("data/oorlogsslachtoffers.json", function (json) {
        console.log(json)
